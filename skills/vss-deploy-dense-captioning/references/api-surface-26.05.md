...
Cosmos Reason3 this is usually `nim_nvidia_cosmos3-nano-reasoner_modelopt-fp8-final_format_fix`; backend
...
curl -X POST "$BASE_URL/v1/chat/completions" -H "Authorization: Bearer $API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "nim_nvidia_cosmos3-nano-reasoner_modelopt-fp8-final_format_fix",
    "messages": [
      {
        "role": "user",
        "content": [
