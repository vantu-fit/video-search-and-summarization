...
| VLM / RT-VLM | Deploys **`vss-rtvi-vlm`** in this release. The RT-VLM pod loads the integrated Cosmos Reason3 Nano FP8 checkpoint; the profile does **not** deploy a separate Cosmos VLM NIM by default. | `nim_nvidia_cosmos3-nano-reasoner_modelopt-fp8-final_format_fix` |
...
| `rtvi.vss-rtvi-vlm.useSharedNim` | `false` | Load the integrated checkpoint in the RT-VLM pod. Sets `MODEL_PATH=ngc:nim/nvidia/cosmos3-nano-reasoner:modelopt-fp8-final_format_fix` and `VLM_MODEL_TO_USE=cosmos-reason3`. |
| `rtvi.vss-rtvi-vlm.modelPath` | `ngc:nim/nvidia/cosmos3-nano-reasoner:modelopt-fp8-final_format_fix` | Integrated RT-VLM checkpoint path used when `useSharedNim=false`. |
...
| **`global.vlmName`** | **`nim_nvidia_cosmos3-nano-reasoner_modelopt-fp8-final_format_fix`** | Same for VLM (**`VLM_NAME`**, **`VIA_VLM_OPENAI_MODEL_DEPLOYMENT_NAME`**). |
...
helm upgrade --install vss-lvs ./dev-profile-lvs \
  ...
  --set-string global.vlmName="nim_nvidia_cosmos3-nano-reasoner_modelopt-fp8-final_format_fix" \
